...
const Cart = () => {
  ...
  useEffect(() => {
    console.log("Effect triggered, cartItems =", cartItems);
  
    const tempData = [];
  
    for (const items in cartItems) {
      console.log("Item ID:", items, cartItems[items]);
      for (const item in cartItems[items]) {
        console.log("-- Size:", item, "Qty:", cartItems[items][item]);
       ...
       <div>
         {
           cartData.map((item, index) => {
            ...
            return(
              <div key={index} className='py-4 border-t text-gray-700 grid grid-cols-[4fr_0.5fr] sm:grid-cols-[4fr_2fr_0.5fr] items-center gap-4'>
